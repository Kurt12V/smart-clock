...
bool BluetoothSubscriptionManager::subscribe(const char* topic)
{
    ...
    _topics[_count] = topic;
    _count++;

    return true;
}

bool BluetoothSubscriptionManager::unsubscribe(const char* topic)
{
    if (topic == nullptr)
        return false;
...
bool BluetoothSubscriptionManager::isSubscribed(const char* topic) const
{
    if (topic == nullptr)
        return false;
